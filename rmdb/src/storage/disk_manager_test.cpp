   ...
   public:
    ...
    void SetUp() override {
        ...
        try {
            ...
            if (!disk_manager_->is_dir(TEST_DB_NAME)) {
                throw std::runtime_error("Failed to create test directory: " + TEST_DB_NAME);
            }
            // 进入测试目录
            if (chdir(TEST_DB_NAME.c_str()) < 0) {
                throw std::runtime_error("Failed to change directory to: " + TEST_DB_NAME);
            }
        } catch (const std::exception& e) {
            std::cerr << "Error in SetUp: " << e.what() << std::endl;
            throw;  // Rethrow to make the error visible and to stop the test
        }
    }
...
TEST_F(DiskManagerTest, FileOperation) {
    ...
    for (size_t i = 0; i < filenames.size(); i++) {
        ...
        if (disk_manager_->is_file(filename)) {
            std::cout << "Deleting leftover file: " << filename << std::endl;
            disk_manager_->destroy_file(filename);
        }
        // 测试异常：如果没有创建文件就打开文件
        try {
            std::cout << "Attempting to open nonexistent file: " << filename << std::endl;
            disk_manager_->open_file(filename);
            assert(false);  // 如果未抛出 FileNotFoundError，则会执行这里，导致测试失败
        } catch (const FileNotFoundError &e) {
            std::cout << "Caught expected FileNotFoundError: " << e.what() << std::endl;
        }
        // 创建文件
        disk_manager_->create_file(filename);
        EXPECT_TRUE(disk_manager_->is_file(filename));  // 检查是否创建文件成功
        try {
            disk_manager_->create_file(filename);
            assert(false);
        } catch (const FileExistsError &e) {
            std::cout << "Caught expected FileExistsError: " << e.what() << std::endl;
        }
        // 打开文件
    ...
    for (auto &entry : fd2name) {
        ...
        disk_manager_->close_file(fd);
        disk_manager_->destroy_file(filename);
        EXPECT_FALSE(disk_manager_->is_file(filename));  // 检查是否删除文件成功
        try {
            disk_manager_->destroy_file(filename);
            assert(false);
        } catch (const FileNotFoundError &e) {
            std::cout << "Caught expected FileNotFoundError on destroy: " << e.what() << std::endl;
        }
    }
}
